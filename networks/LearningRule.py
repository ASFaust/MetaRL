...
class LearningRule:
    ...
    def get_random_params(batch_dim,
                          ...
                          device='cuda'):
        return {
            "node_network": LearningRuleNetwork.get_random_params(
                batch_dim,
                input_dim=node_info_dim + signal_dim + 1 + 1, # one for action flags, one for reward
            ...
            "connection_network": LearningRuleNetwork.get_random_params(
                batch_dim,
                input_dim=node_info_dim * 2 + signal_dim + 1 + 1 + 1,
    ...
    def __init__(self, params):
        self.batch_dim = params["node_network"]["w_output"].shape[0]
